...
export const utilizationThroughPutChart = () => {
  const ctx = document.getElementById("utilizationThroughPutChart");
  if (!ctx) return;
  ...
  fetch("/api/utilization-data")
    ...
    .then((data) => {
      const btp = data.map((item) => item.btp),
        ytp = data.map((item) => item.ytp),
        stp = data.map((item) => item.stp);
      ...
      const myData = [
        {
          label: "Data Berth Through Put",
          data: btp,
          backgroundColor: "#99FFFF",
        },
        {
          label: "Data Yard Through Put",
          data: ytp,
          backgroundColor: "#273BE2",
        },
        {
          label: "Data Shed Through Put",
          data: stp,
      ...
      new Chart(ctx, {
        ...
        options: {
          ...
          plugins: {
            ...
            htmlLegend: {
              containerId: "utilization-through_put-data-legend",
            },
            tooltip: {
              callbacks: {
            ...
            afterUpdate(c, args, options) {
              ...
              items.forEach((item) => {
                ...
                const theValue = totalValue.reduce((a, b) => a + b),
                  valueText = document.createTextNode(`${theValue} Ton/M2`),
